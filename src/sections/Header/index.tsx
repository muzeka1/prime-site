...
export default function Header() {
  ...
  const [requestFormIsOpen, setRequestFormIsOpen] = useState<boolean>(false)
      ...
      <div className={styles.nav_bar_container}>
        <div className={styles.nav_bar}>
          ...
          <nav className="hidden md:flex gap-6">
            ...
            <Link href="/about" className={styles.navButton}>О компании</Link>
            <button onClick={()=>setContactsIsOpen(true)} className={styles.navButton}>Контакты</button>
          </nav>

          <div className="hidden md:block">
            <button onClick={()=>setRequestFormIsOpen(true)}>Оставить заявку</button>
          </div>
          <MobileMenu />
        </div>
      </div>
